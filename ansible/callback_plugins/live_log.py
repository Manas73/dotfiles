"""Print a rolling buffer of the packages live log during async polls.

Long yay/mise installs redirect their stdout to packages_live_log and
run with async+poll. Ansible would otherwise stay silent until the task
finishes. This notification callback tails the log on each poll: at most
``max_lines`` new lines per burst, so the play output is a short moving
window rather than a firehose. An empty poll prints a heartbeat so a
quiet command still looks alive.

Enable via ansible.cfg ``callbacks_enabled = live_log``.
Override the path with ANSIBLE_PACKAGES_LIVE_LOG, the window size with
ANSIBLE_PACKAGES_LIVE_LOG_LINES.
"""
...
DOCUMENTATION = r"""
  ...
  description:
    - On each async poll (and when a task finishes), print any new lines
      from the packages live log file, capped at max_lines per burst.
    - Empty polls print a heartbeat so long quiet commands do not look stuck.
  options:
    log_path:
