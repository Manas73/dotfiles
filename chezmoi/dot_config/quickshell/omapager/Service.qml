...
Item {
  ...
  // Toasts only on this output. DP-1 is alfred's left monitor; fall back to
  // the first screen if that name is missing (lid closed, cable pulled).
  property string screenName: "DP-1"

  readonly property var notificationScreen: {
    var target = String(screenName || "")
  ...
  // ------------------------------------------------------------- surface
  //
  // One layer, on `screenName` only. Full-screen and fixed: a surface that
  ...
  // pixel outside the deck click-through.
    PanelWindow {
      id: surface
      screen: service.notificationScreen
