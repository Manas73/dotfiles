...
BarWidget {
  ...
  // This shell has no plugin loader, so the widget has to register itself the
  // way Panel.qml does. Without that, every monitor's copy thinks it owns
  // `omapager.panel` (empty registry → ipcOwner) and Super+N cannot find the
  // focused bar.
  ...
  function syncModuleRegistry() {
    ...
    bar.registerModuleWidget(moduleName, pager)
    _registeredBar = bar
    _registeredName = moduleName
    _registeredItem = pager
  }
  ...
  function invokeFocused(method) {
    ...
    if (typeof pager[fn] === "function") return pager[fn]()
  }
  ...
  // Default on here: this bar has no centre-hover reveal for inactive
  // indicators, so a collapsing slot would hide the only click target.
  ...
  // Left opens the panel, right silences. Matches the rest of this bar:
  // a plain click on a status glyph opens its panel, and the old notify
  // widget used the same mapping.
  function pressed(buttonCode) {
    if (buttonCode === Qt.RightButton) {
      if (quiet) letEverythingThrough()
      else toggleSilence()
    } else {
      togglePanel()
    }
  ...
  IpcHandler {
    enabled: pager.ipcOwner
    target: "omapager.panel"
    function open(): string { pager.invokeFocused("open"); return "open" }
    ...
    function close(): string { pager.invokeFocused("close"); return "closed" }
    function toggle(): string { pager.invokeFocused("toggle"); return "ok" }
  ...
  Row {
    ...
    Indicator {
      ...
      tooltipText: "Notifications silenced - click for options, right-click to allow them"
    }
    ...
    Indicator {
      ...
      tooltipText: pager.globalSnoozed
                   ? ("Everything snoozed, back " + pager.waking(pager.globalUntil)
                      + " - click for options, right-click to let them through")
    ...
    Indicator {
      ...
      quiet: true
      tooltipText: "Notifications - click for options, right-click to silence"
    }
  }
