...
Panel {
  ...
  // The share card is its own panel plugin (wifi-qr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("wifi-qr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = [Quickshell.shellDir + "/bin/omarchy-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = [Quickshell.shellDir + "/bin/omarchy-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (speedtest) so a
  ...
  Process {
    id: detailsProc
    command: [Quickshell.shellDir + "/bin/omarchy-network-status", "--verbose"]
