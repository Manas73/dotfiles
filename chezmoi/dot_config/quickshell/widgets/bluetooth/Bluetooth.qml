...
Panel {
  ...
  function deviceCommand(action, address) {
    return [Quickshell.shellDir + "/bin/omarchy-bluetooth-device", action, address]
  }
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-bluetooth-power", adapter.enabled ? "off" : "on"])
  }
