...
Panel {
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = [Quickshell.shellDir + "/bin/omarchy-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", Quickshell.shellDir + "/bin/omarchy-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = [Quickshell.shellDir + "/bin/omarchy-display-text-size", String(px)]
  ...
  Process {
    id: stateProc
    command: [Quickshell.shellDir + "/bin/omarchy-monitor-state"]
