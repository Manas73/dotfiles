...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = [Quickshell.shellDir + "/bin/omarchy-network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: [Quickshell.shellDir + "/bin/omarchy-network-status"]
