...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "wifi-qr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? [Quickshell.shellDir + "/bin/omarchy-network-qr", "--meta", requestedIface]
      : [Quickshell.shellDir + "/bin/omarchy-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = [Quickshell.shellDir + "/bin/omarchy-network-password", iface]
    pwProc.running = true
  }
