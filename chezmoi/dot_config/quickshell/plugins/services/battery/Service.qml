...
Item {
  ...
  function sendLowBatteryWarning(level) {
    ...
    warningProcess.command = [Quickshell.shellDir + "/bin/omarchy-battery-low",
      String(level)
    ]
  ...
  function runPendingPowerProfile() {
    powerProfileProcess.command = [Quickshell.shellDir + "/bin/omarchy-powerprofiles-set", pendingPowerSource]
    pendingPowerSource = ""
