...
BarIndicator {
  ...
  function openReminderFlow() {
    Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-reminder", "-i"])
  }
  ...
  Process {
    id: jsonProc
    command: [Quickshell.shellDir + "/bin/omarchy-reminder", "show", "--json"]
  ...
  onPressed: function() {
    if (root.reminderCount > 0) Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-reminder", "show"])
