...
Item {
  ...
  property string omarchyConfigDir: home + "/.config/quickshell"
  ...
  function refreshTransparentForeground() {
    ...
    transparentForegroundProc.command = [Quickshell.shellDir + "/bin/omarchy-bar-text-color",
      root.position,
      String(root.barSize),
  ...
  FileView {
    path: root.home + "/.config/quickshell/theme"
    watchChanges: true
    printErrors: false
  ...
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/quickshell/toggles/bar-off ]] && echo yes || echo no"]
