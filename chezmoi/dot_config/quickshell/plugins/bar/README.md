...
- The bar receives its config from the host shell as a `barConfig` property; the host loads it from `~/.config/quickshell/shell.json` (or `config/omarchy/shell.json` when the user has no file).
...
| `omarchy.tray` | System tray | left on chevron = toggle drawer · right on chevron = manage |
