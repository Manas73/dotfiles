...
Item {
  ...
  function sendFile(peer) {
    ...
    if (target === "") return
    Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-tailscale-send", target])
  }
  ...
  function openAuthUrlFrom(text, allowFallback) {
    ...
    if (url !== "") {
      ...
      _loginInProgress = false
      loginTimeoutTimer.stop()
      Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-launch-browser", url])
      return true
    }
    return false
  }
