...
`omarchy-agent-usage-update` writes to `~/.local/state/quickshell/agents/usage/`
...
Each agent is one JSON record in `~/.local/state/quickshell/agents/usage/`,
...
configuration in `~/.config/quickshell/agents/fireworks.json`:
