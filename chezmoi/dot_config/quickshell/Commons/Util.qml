...
QtObject {
  ...
  function bin(name) {
    return binDir + "/" + String(name || "")
  }

  function withBinPath(command) {
    return "export PATH=" + shellQuote(binDir) + ":\"$PATH\"; " + String(command || "")
  }

  function execDetached(command) {
    Quickshell.execDetached(["bash", "-lc", withBinPath(command)])
  }
