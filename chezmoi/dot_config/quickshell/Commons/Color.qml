...
QtObject {
  ...
  readonly property string bundledThemePath: Quickshell.shellDir + "/defaults/theme"
  ...
  // Prefer ~/.config/quickshell/theme (matugen or a hand-written theme).
  // Fall back to the bundled defaults so the shell starts without a theme dir.
  // Watched so wallpaper-driven regenerations apply without an IPC push.
  property bool userColorsLoaded: false
  property bool userShellLoaded: false
  ...
  property FileView colorsFile: FileView {
    ...
    watchChanges: true
    printErrors: false
    onLoaded: { root.userColorsLoaded = true; root.loadColors(text()) }
    onLoadFailed: { root.userColorsLoaded = false; bundledColorsFile.reload() }
    onFileChanged: reload()
  }
  property FileView bundledColorsFile: FileView {
    path: root.bundledThemePath + "/colors.toml"
    watchChanges: false
    printErrors: false
    onLoaded: if (!root.userColorsLoaded) root.loadColors(text())
  }
  property FileView shellFile: FileView {
    ...
    watchChanges: true
    printErrors: false
    onLoaded: { root.userShellLoaded = true; root.loadShell(text()) }
    onLoadFailed: { root.userShellLoaded = false; bundledShellFile.reload() }
    onFileChanged: reload()
  }
  property FileView bundledShellFile: FileView {
    path: root.bundledThemePath + "/shell.toml"
    watchChanges: false
    printErrors: false
    onLoaded: if (!root.userShellLoaded) root.loadShell(text())
    onLoadFailed: if (!root.userShellLoaded) root.loadShell("")
  }
