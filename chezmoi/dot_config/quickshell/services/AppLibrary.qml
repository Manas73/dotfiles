...
Item {
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.omarchyPath + "/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-shell", "osd", "close"])
  ...
  FileView {
    path: root.omarchyPath + "/defaults/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached([Quickshell.shellDir + "/bin/omarchy-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
