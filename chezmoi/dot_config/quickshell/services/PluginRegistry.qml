...
QtObject {
  ...
  property string pluginsDir: home + "/.config/quickshell/local-plugins"
  ...
  property bool pluginWatchSupported: false

  property Process initProcess: Process {
    onExited: {
      watcherCheck.running = true
      registry.rescan()
    }
  }

  property Process watcherCheck: Process {
    command: ["bash", "-c", "command -v inotifywait >/dev/null"]
    onExited: function(code) {
      registry.pluginWatchSupported = code === 0
      if (registry.pluginWatchSupported) localPluginWatcher.running = true
    }
  }
  ...
  property Process localPluginWatcher: Process {
    ...
    onExited: {
      if (registry.pluginWatchSupported) localPluginWatcherRestart.restart()
    }
  }

  property Timer localPluginWatcherRestart: Timer {
    interval: 1000
    onTriggered: if (registry.pluginWatchSupported) localPluginWatcher.running = true
  }
